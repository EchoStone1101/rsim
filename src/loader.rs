...
pub mod loader {
    ...
    impl Loader {
        ...
        pub const STACK_BOTTOM: u64 = 0x80000000u64;
        pub const STACK_SZ: usize = 0x100000;
        ...
        pub fn load(&self) -> Result<Program, SimError> {
                ...
                Ok(output) => {
                    ...
                    for line in output.split('\n') {
                        ...
                        if !items.is_empty() && items[3].eq("FUNC") {
                            ...
                            if items.last().unwrap().trim().eq("puts") {
            ...
            vmas.push(VMA{
                // Use 1MB stack
                lower_bound: Loader::STACK_BOTTOM - Loader::STACK_SZ as u64,
                size: Loader::STACK_SZ as u64,
                readable: true,
                writeble: true,
                executable: false,
                memory: {
                    let mut data = vec![0u8; Loader::STACK_SZ];
                    data[Loader::STACK_SZ-16..Loader::STACK_SZ-8].copy_from_slice(&HLT_ADDR.to_be_bytes());
                    data
                },
            });
            ...
            let mut registers: Vec<Register> = (0..32).into_iter()
                ...
                .collect();
            registers[RegID::X2.encode() as usize].write(Loader::STACK_BOTTOM - 16);
    ...
    impl fmt::Display for ELFArch {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            match self {
                ELFArch::Rv64I => write!(f, "\"rv64i2p0_m2p0_a2p0_f2p0_d2p0\""),
                _ => Ok(()),
            }
        }
    }
